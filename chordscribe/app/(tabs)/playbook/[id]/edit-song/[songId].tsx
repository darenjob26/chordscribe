...
export default observer(function EditSongScreen() {
  const router = useRouter()
  const { id, songId } = useLocalSearchParams<{ id: string; songId: string }>()
  const { colors } = useTheme()
  const insets = useSafeAreaInsets()
  const { sections, deleteSection, clearSections, addSection } = useSong()
  ...
  useEffect(() => {
    ...
    if (song) {
      ...
      setSongKey(song.key)
      // Clear existing sections and add the song's sections
      clearSections()
      song.sections.forEach(section => addSection(section))
    }
  }, [songId])
  ...
  return (
    <View className="flex-1 p-4" style={{ paddingTop: headerHeight }}>
      <View className="flex-row items-center mb-4">
        ...
        <Text className="text-2xl font-bold" style={{ color: colors.text }}>Edit Song</Text>
      </View>
